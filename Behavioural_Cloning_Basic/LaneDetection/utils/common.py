...
def get_model(cfg):
    ds = str(getattr(cfg, 'dataset', '')).lower()
    if   ds == 'tusimple': module_name = 'model.model_tusimple'
    elif ds == 'culane'  : module_name = 'model.model_culane'
    else:
        raise ValueError(f"Unknown dataset '{cfg.dataset}'. Expected 'Tusimple' or 'CULane'.")

    mod = importlib.import_module(module_name)

    # 1) Wenn das Modul bereits eine passende Fabrikfunktion hat, nutze sie:
    if hasattr(mod, 'get_model'):
        print(f"[get_model] Using {module_name}.get_model(cfg)")
        return mod.get_model(cfg)  # Achtung: kann schon .cuda() aufrufen

    # 2) Sonst eine passende Klasse finden …
    candidates = ['Model', 'ParsingNet', 'SegModel', 'parsingNet']
    Model = None
    ...
    if Model is None:
        raise ImportError(
            f"Could not find a model class in {module_name}. "
            f"Tried candidates: {candidates}."
        )

    print(f"[get_model] Using {module_name}.{Model.__name__} with kwargs")

    # 3) … und mit korrekten Keyword-Argumenten instanziieren:
    kwargs = dict(
        pretrained=True,
...
def get_train_loader(cfg):
    if TrainCollect is None:
        raise ImportError(
        "nvidia.dali ist nicht installiert. Das wird nur fürs TRAINING benötigt. "
        "Für demo.py / Inferenz bitte ohne DALI nutzen oder nvidia-dali-cudaXX installieren."
        )
    if cfg.dataset == 'CULane':
